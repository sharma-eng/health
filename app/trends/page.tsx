...
export default function TrendsPage() {
  ...
  const flatTrendData = useMemo(() => {
    if (!data?.insights?.biomarkerTrends?.length) return [];
    return data.insights.biomarkerTrends.flatMap((trend) =>
      trend.points.map((p) => ({
        date: p.date,
        value: p.value,
    ...
    <main className="flex flex-1 flex-col gap-6 py-4 sm:py-6">
      ...
      <section className="glass-panel flex flex-col gap-4 p-5 sm:p-6">
        ...
        <div className="mt-2 min-h-[260px] rounded-2xl border border-slate-800/80 bg-slate-950/60 p-3 sm:p-4">
          ...
          {data && flatTrendData.length > 0 ? (
            <div className="h-64 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={flatTrendData}>
                  <CartesianGrid
            ...
            <div className="flex h-64 items-center justify-center text-xs text-slate-400">
              {loading
                ? "Fetching trend data..."
                : "No trend data yet. Upload a report, then load trends."}
            </div>
          )}
        </div>
      </section>
